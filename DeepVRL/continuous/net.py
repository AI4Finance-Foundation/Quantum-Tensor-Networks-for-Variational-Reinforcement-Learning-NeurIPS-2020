...
class ActorSAC(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim, if_use_dn=False):
        ...
        self.net_state = nn.Sequential(nn.Linear(state_dim, inp_dim), nn.ReLU(),
                                       nn.Linear(mid_dim, mid_dim), nn.ReLU())
        self.net_a_avg = nn.Sequential(nn.Linear(mid_dim, action_dim))  # the average of action
        self.net_a_std = nn.Sequential(nn.Linear(mid_dim, action_dim))  # the log_std of action
    ...
    def get_action(self, state):
        ...
        return Normal(a_avg, a_std).rsample().tanh()  # re-parameterize
    ...
    def get_action_logprob(self, state):
        ...
        normal = Normal(a_avg, a_std)
        a_pretan = normal.rsample()  # action.tanh()
        ...
        # same as below:
        a_tan = a_pretan.tanh()
        log_prob = normal.log_prob(a_pretan)
...
class CriticREDQ(nn.Module):  # ensemble N critic 
    ...
    def get_q_values_choose(self, state, action, choose):
        tensor_sa = torch.cat((state, action), dim=1)
        tensor_qs = [getattr(self, f'critic{choose[critic_id]:02}').net(tensor_sa)  # criticID(tensor_sa)
                     for critic_id in range(choose.shape[0])]
        #tensor_qs = [self.critic_list[choose[critic_id]].net(tensor_sa)  
        #             for critic_id in range(choose.shape[0])]
        tensor_qs = torch.cat(tensor_qs, dim=1)
        return tensor_qs
