...
def get_gym_env_info(env, if_print) -> (str, int, int, int, int, bool, float):
    ...
    env_name = getattr(env.env, 'env_name', None)
    #env_name = 'simple_spread'
    # env_name = env.unwrapped.spec.id if env_name is None else env_name

    if isinstance(env.env.observation_space, gym.spaces.discrete.Discrete):
        raise RuntimeError("| <class 'gym.spaces.discrete.Discrete'> does not support environment with discrete observation (state) space.")
    if env.env.unwrapped.spec.id == 'MARL':
        state_shape = env.env.observation_space[0].shape
    else:
        state_shape = env.env.observation_space.shape
    state_dim = state_shape[0] if len(state_shape) == 1 else state_shape  # sometimes state_dim is a list

    target_return = getattr(env, 'target_return', None)
    target_return_default = getattr(env.env.spec, 'reward_threshold', None)
...
def build_env(env, if_print=False):
    env_name = getattr(env.env, 'env_name', env)
    # print(env_name)
